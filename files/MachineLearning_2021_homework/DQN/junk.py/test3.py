...
class Agent:
        ...
        if len(self.trajectory) >= 8:
            ...
            if rwd == -1:  # traceback: store transitions of several actions before losing a match
                ...
                for i in range(1, iter_traceback_len):
                    ...
                    self.memorysys.store(stt_trbc, act_trbc, rwd_trbc, stt_new_trbc, tmnt_trbc)  # store into memory
            """

    def step(self, stt, rwd, tmnt):
        # follow the network
        result = self.target_net(stt).view(-1)
        act = result.argmax()
        if(self.mode) == "train":
            if np.random.rand() < self.epsilon:
                act = np.random.randint(low=0,high=self.action_n)
            self.trajectory += [stt, rwd, tmnt, act]
            self.store(rwd)
    ...
    def learn_dqn(self):
        ...
        for i in range(0, batch_size):
            stt = batch.iat[i,0]
            stt_new = batch.iat[i, 3]
            act = torch.tensor(batch.iat[i,1])
            rwd = torch.tensor(batch.iat[i,2])
            tmnt = batch.iat[i,4]
            if int(tmnt) == 0:
        ...
        self.epsilon = max(self.epsilon - 2*1e-5, 0.05)
